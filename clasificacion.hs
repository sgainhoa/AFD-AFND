...
clasificacionAf:: Af-> IO ()
clasificacionAf (est,alf,tr,ini,dob)
	|(clasificacionAf_aux (est,alf,tr,(head est),dob) est )== True = putStrLn "El Af indicado es un AFD"
	|otherwise=putStrLn "El Af indicado es un AFND"
